        ...
        if obj.abline.on
            ...
            for line_ind=1:length(obj.abline.intercept)
                ...
                if ~isnan(obj.abline.intercept(line_ind))
                    %abline
                    plot(xl,xl*obj.abline.slope(line_ind)+obj.abline.intercept(line_ind),obj.abline.style{line_ind},'LineWidth',obj.abline.line_width,'Parent',ca);
                else
                    if ~isnan(obj.abline.xintercept(line_ind))
                        ...
                        plot([obj.abline.xintercept(line_ind) obj.abline.xintercept(line_ind)],yl,obj.abline.style{line_ind},'LineWidth',obj.abline.line_width,'Parent',ca);
                    else
                        if ~isnan(obj.abline.yintercept(line_ind))
                            %hline
                            plot(xl,[obj.abline.yintercept(line_ind) obj.abline.yintercept(line_ind)],obj.abline.style{line_ind},'LineWidth',obj.abline.line_width,'Parent',ca);
                        else
                            temp_x=linspace(xl(1),xl(2),500);
                            plot(temp_x,obj.abline.fun{line_ind}(temp_x),obj.abline.style{line_ind},'LineWidth',obj.abline.line_width,'Parent',ca);
                        end
                    end
                end
            end
